...
__global__ void histogram_kernel(char *data, unsigned int *histogram,
                                 unsigned int length, unsigned int num_bins)
{
...
int main(int argc, char *argv[])
{
    ...
    start_gpu = get_time();
    histogram_kernel<<<blocksPerGrid, threadsPerBlock, sizeof(unsigned int)*5>>>(text_d, histogram_d, len, 5); //same nof as the regular histogram
    CHECK_KERNELCALL();
